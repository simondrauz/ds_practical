...
More recent work has embraced graph-based and attention-based architectures to better capture structured interactions. DESIRE \citep{lee2017desire} and Trajectron++ \citep{salzmann2020trajectron+} combine probabilistic reasoning with multi-agent interaction modelling. Trajectron++ in particular offers a principled framework for heterogeneous, multi-agent trajectory prediction: it represents the scene as a dynamic graph, uses recurrent encoders to model each agent's history, and employs conditional variational autoencoders (CVAEs) to produce a distribution over future trajectories. It further supports integration of map and semantic context, and accommodates multiple agent types including pedestrians, cars, and cyclists, making it well-suited to complex real-world datasets.
...
\paragraph{Approach 1: Mutual-information elbow followed by VIF.} We first ranked all $18$ candidate features by their mutual information (MI) with the target \citep{kraskov2004estimating}, which measures the strength of the (possibly non-linear) statistical dependence between each feature and the prediction error. Inspecting the ranked MI curve revealed an elbow, and we discarded the low-signal features lying below it, reducing the set from $18$ to $10$. We then applied a variance inflation factor (VIF) filter \citep{kutner2005applied} with a threshold of $5$ to remove residual multicollinearity, which further reduced the set to $4$ features: \texttt{std\_speed}, \texttt{mean\_acceleration}, \texttt{max\_speed}, and \texttt{heading\_change\_per\_sec}. The threshold was used as a conservative operational cutoff rather than a universal criterion, since fixed VIF rules should be treated as diagnostics rather than hard statistical laws \citep{obrien2007vif}. This procedure retains only motion features; all scene-context features fall below the MI elbow, indicating that, considered individually, they carry comparatively little information about per-trajectory prediction error. All four retained features are highly statistically significant ($p \ll 0.01$).
...
\begin{table}[t]
  \centering
  \caption{Out-of-fold model-variant comparison on the original \texttt{ml\_ade} scale for models fitted on the main MI\,+\,VIF feature set. Bold rows indicate the selected variants carried forward for interpretation.}
...
We compared OPTICS \citep{ankerst1999optics} and HDBSCAN \citep{campello2015hdbscan} within each performance group and selected candidates using three criteria: Density-Based Clustering Validation (DBCV) \citep{moulavi2014density}, the share of points labelled as noise, and semantic quality after mapping the clusters back to the raw trajectory characteristics. DBCV is an internal score for density-based cluster structure with range $[-1,1]$; values near $+1$ indicate well-separated dense groups, while values near $0$ indicate ambiguous density structure. Appendix~\ref{app:dbcv_details} gives the formal definition used to read this score.
...
Table~\ref{tab:rq2_cluster_evidence} reports the cluster-level trajectory characteristics and signed XGBoost effects used for the main-text regime interpretation. It includes all eight non-noise clusters from the selected XGBoost--OPTICS solution and the three corresponding noise groups, because the noise groups explain much of the continuous boundary structure.

\clearpage
\begin{landscape}
\begin{table}[H]
  \centering
  \caption{Complete XGBoost--OPTICS cluster evidence behind the regime interpretations. Each feature cell reports \emph{raw}, the median trajectory-characteristic value, and \emph{eff.}, the mean signed XGBoost SHAP effect on the \texttt{ml\_ade\_log} target. The total-effect column reports the median summed signed effect, and heading change is measured in degrees per second.}
  \label{tab:rq2_cluster_evidence}
  \scriptsize
  \renewcommand{\arraystretch}{1.20}
  \newcommand{\rawEffectCell}[2]{\begin{tabular}[c]{@{}r@{}}raw $#1$\\eff. $#2$\end{tabular}}
  {\setlength{\tabcolsep}{1.4pt}
  \begin{tabular}{@{}p{0.055\linewidth}p{0.155\linewidth}p{0.100\linewidth}p{0.075\linewidth}p{0.075\linewidth}p{0.115\linewidth}p{0.115\linewidth}p{0.115\linewidth}p{0.115\linewidth}@{}}
    \toprule
    \textbf{Cluster} & \textbf{Semantic label} & \textbf{Size} & \begin{tabular}[c]{@{}l@{}}\textbf{Median}\\\textbf{\texttt{ml\_ade}}\end{tabular} & \begin{tabular}[c]{@{}l@{}}\textbf{Total}\\\textbf{effect}\end{tabular} & \begin{tabular}[c]{@{}c@{}}\texttt{std\_speed}\\raw/eff.\end{tabular} & \begin{tabular}[c]{@{}c@{}}\texttt{max\_speed}\\raw/eff.\end{tabular} & \begin{tabular}[c]{@{}c@{}}\texttt{heading\_change}\\\texttt{\_per\_sec}\\raw/eff.\end{tabular} & \begin{tabular}[c]{@{}c@{}}\texttt{mean}\\\texttt{\_acceleration}\\raw/eff.\end{tabular} \\
    \midrule
    \multicolumn{9}{@{}l}{\textbf{Easy}} \\
    \midrule
    C0 & Slow turners & $5{,}323$ ($33.6\%$) & $0.083$ & $-0.282$ & \rawEffectCell{0.037}{-0.128} & \rawEffectCell{0.124}{-0.223} & \rawEffectCell{45.714}{+0.041} & \rawEffectCell{0.000}{+0.023} \\
    C1 & Straight walkers & $5{,}754$ ($36.4\%$) & $0.199$ & $-0.156$ & \rawEffectCell{0.066}{-0.081} & \rawEffectCell{1.412}{+0.042} & \rawEffectCell{1.934}{-0.121} & \rawEffectCell{-0.003}{-0.000} \\
    Noise & Easy residual & $4{,}746$ ($30.0\%$) & $0.238$ & $-0.090$ & \rawEffectCell{0.128}{-0.035} & \rawEffectCell{1.349}{-0.005} & \rawEffectCell{7.047}{-0.036} & \rawEffectCell{-0.002}{-0.014} \\
    \addlinespace
    \multicolumn{9}{@{}l}{\textbf{Medium}} \\
    \midrule
    C0 & Ordinary medium & $4{,}975$ ($59.8\%$) & $0.551$ & $+0.008$ & \rawEffectCell{0.122}{-0.033} & \rawEffectCell{1.525}{+0.072} & \rawEffectCell{4.641}{-0.028} & \rawEffectCell{-0.002}{+0.007} \\
    C1 & Small slow-turner edge & $19$ ($0.2\%$) & $0.424$ & $-0.207$ & \rawEffectCell{0.067}{-0.125} & \rawEffectCell{0.190}{-0.226} & \rawEffectCell{48.589}{+0.107} & \rawEffectCell{0.005}{+0.037} \\
    Noise & Boundary cases & $3{,}326$ ($40.0\%$) & $0.726$ & $+0.187$ & \rawEffectCell{0.224}{+0.109} & \rawEffectCell{1.491}{+0.054} & \rawEffectCell{10.542}{+0.036} & \rawEffectCell{0.003}{-0.017} \\
    \addlinespace
    \multicolumn{9}{@{}l}{\textbf{Hard}} \\
    \midrule
    C0 & Erratic turners & $31$ ($1.1\%$) & $1.942$ & $+0.675$ & \rawEffectCell{0.452}{+0.488} & \rawEffectCell{1.244}{+0.070} & \rawEffectCell{12.310}{+0.126} & \rawEffectCell{-0.112}{-0.019} \\
    C1 & Erratic turners & $68$ ($2.5\%$) & $1.584$ & $+0.507$ & \rawEffectCell{0.389}{+0.381} & \rawEffectCell{1.194}{+0.075} & \rawEffectCell{27.163}{+0.149} & \rawEffectCell{-0.121}{-0.077} \\
    C2 & Erratic turners & $118$ ($4.3\%$) & $1.607$ & $+0.450$ & \rawEffectCell{0.288}{+0.233} & \rawEffectCell{1.074}{+0.066} & \rawEffectCell{19.527}{+0.130} & \rawEffectCell{-0.029}{+0.012} \\
    C3 & Fast and turning & $473$ ($17.2\%$) & $1.500$ & $+0.325$ & \rawEffectCell{0.127}{+0.009} & \rawEffectCell{1.551}{+0.136} & \rawEffectCell{10.526}{+0.146} & \rawEffectCell{-0.004}{+0.031} \\
    Noise & High-error residual & $2{,}053$ ($74.8\%$) & $1.934$ & $+0.601$ & \rawEffectCell{0.382}{+0.361} & \rawEffectCell{1.539}{+0.131} & \rawEffectCell{18.805}{+0.123} & \rawEffectCell{0.017}{-0.004} \\
    \bottomrule
  \end{tabular}
  }
\end{table}
