...
\begin{table}[H]
  \centering
  \caption{Out-of-fold errors of the selected MI\,+\,VIF meta-models stratified by performance group, on the original \texttt{ml\_ade} scale (in metres). Bias is the mean of predicted minus actual \texttt{ml\_ade}.}
...
\begin{figure}[H]
  ...
  \caption{SHAP beeswarm for the XGBoost model on the VIF-only robustness feature set (log-target scale), analogous to Figure~\ref{fig:appendix_shap_beeswarm}.}
...
\begin{figure}[H]
  ...
  \caption{Two-dimensional centred partial dependence surfaces for all feature pairs of the VIF-only robustness feature set (XGBoost, log-target scale). The near-vertical contours in the panels pairing \texttt{std\_speed} with scene and social features indicate the absence of notable motion--scene interactions.}
